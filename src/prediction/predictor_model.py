...
class Forecaster:
    ...
    model_name = "Double Expoential Smoothing Forecaster"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Construct a new Double Expoential Smoothing Forecaster
    ...
    def fit(
        ...
        self.levels_and_trends = {}

        for id, series in zip(all_ids, all_series):
            if self.history_length:
                series = series[-self.history_length :]
            level, trend = self._fit_on_series(history=series, data_schema=data_schema)
            self.levels_and_trends[id] = (level, trend)
    ...
    def _fit_on_series(self, history: pd.DataFrame, data_schema):
        """Fit model to given individual series of data using double exponential smoothing"""
        time_series = np.array(history[data_schema.target])
        
        # Initialize the level and trend
        if len(time_series) < self.init_period + 1:
    ...
    def predict(
        ...
        # concatenate all series' forecasts into a Double dataframe
    ...
    def _predict_on_series(self, key_and_future_df):
        """
        Make forecast on given individual series of data using double
        exponential smoothing.
        """
        key, future_df = key_and_future_df

        # Check if the model has been fitted with level and trend for the key
        if key in self.levels_and_trends:
            level, trend = self.levels_and_trends[key]
            forecast_length = len(future_df)
            
            # Generate forecasts using the level and trend
            forecasts = []
            for i in range(forecast_length):
                # For double exponential smoothing, the forecast is level + i * trend
                # where i is the time step ahead from the starting point
                forecast = level + i * trend
                forecasts.append(forecast)
            
            future_df[self.data_schema.target] = forecasts
        else:
            # No model (level and trend) found for the key, so can't forecast for it.
            future_df = None
